...
export default class Tracking extends Component {
    ...
    componentDidMount = async () => {
        ...
        if (this.floor.floorData.length !== 0) {
            ...
            this.showMessage(false, true, false, this.floor.message);
        }
    }
    ...
    rackMonitor = (rackCall) => {
        ...
        axios({
            method: "GET",
            url: "/api/rack/monitor?id=" + this.floorData[floorID].id
        })
            .then((response) => {
                if (response.status === 200 || response.status === 201) {
                    ...
                    if (data.length !== 0) {
                        ...
                        if (temp_client !== null) { // temp_client shouldn't get null value
                            ...
                            for (let i = 0; i < data.length; i++) {
                                ...
                                $(childDiv1).on('click', () => this.animation(data[i].name, data[i].rackid, "show"))
                                this.setState({ loading: false });
    ...
    assetData = (radioCheck, rackId, callStatus) => {
        ...
        axios({ method: "GET", url: "/api/rack/chart?id=" + rackId + "&&key=" + radioCheck })
            .then((response) => {
                if (response.status === 200 || response.status === 201) {
                                            ...
                                            <div className="back from-left">
                                                <div className="ass-info">
                                                    ...
                                                    <span className="ass-val" id='mintemp'>
                                                        {animateData[0] > 0 ? animateData[0].toFixed(1) : 0}</span>
                                                </div>
                                                <div className="ass-info">
                                                    <span>Max.Temp(°C)</span>:
                                                    <span className="ass-val" id='maxtemp'>
                                                        {animateData[1] > 0 ? animateData[1].toFixed(1) : 0}</span>
                                                </div>
                                                <div className="ass-info">
                                                    <span>Avg.Temp(°C)</span>:
                                                    <span className="ass-val" id='avgtemp'>
                                                        {animateData[2] > 0 ? animateData[2].toFixed(1) : 0}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {display === "energy" && (
                                    <div className="card">
                                        <div className="content">
                                            ...
                                            <div className="back from-left">
                                                <div className="ass-info">
                                                    <span>Net.Power(kWh)</span>:
                                                    <span className="ass-val" id='netpower'>
                                                        {animateData[0] > 0 ? animateData[0].toFixed(1) : 0}</span>
                                                </div>
                                                <div className="ass-info">
                                                    <span>Max.Power(kWh)</span>:
                                                    <span className="ass-val" id='maxpower'>
                                                        {animateData[1] > 0 ? animateData[1].toFixed(1) : 0}</span>
                                                </div>
                                                <div className="ass-info">
                                                    <span>Min.Power(kWh)</span>:
